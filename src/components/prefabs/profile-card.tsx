    ...
    return (
        <div className="flex aspect-3/4">
            <div className="group from-secondary to-secondary-light relative flex grow flex-col overflow-hidden rounded-2xl bg-radial p-2 transition-all duration-1000">
                ...
                <div className="group-hover:text-secondary group-hover:fill-secondary relative z-20 flex grow flex-col items-start justify-center gap-2 rounded-2xl fill-white p-4 text-white group-hover:bg-white">
                    <h4 className="text-xl font-bold">{name}</h4>
                    <p className="text-sm">{designation}</p>
                    <p className={cn("hidden text-justify text-sm", showDetails ? "block" : "")}>{about}</p>

                    <div className="flex items-center gap-6 [&>svg]:size-6">
                        {instagram && (
